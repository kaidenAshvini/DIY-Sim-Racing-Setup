...
volatile uint32_t dbgSetEffectCount = 0;
volatile uint32_t dbgConstantCount = 0;
volatile uint32_t dbgOperationCount = 0;
volatile uint32_t dbgBlockFreeCount = 0;

volatile uint8_t dbgLastSetBlock = 0;
volatile uint8_t dbgLastSetType = 0;

volatile int16_t dbgLastMagnitude = 0;

volatile uint8_t dbgLastOpBlock = 0;
volatile uint8_t dbgLastOperation = 0;

volatile uint16_t dbgConstantLen = 0;
volatile int16_t dbgRawConstantMagnitude = 0;
...
{
  dbgOperationCount++;
  dbgLastOpBlock = data->effectBlockIndex;
  dbgLastOperation = data->operation;
...
void FfbReportHandler::FfbHandle_SetEffect(USB_FFBReport_SetEffect_Output_Data_t* data)
{
  ...
  dbgSetEffectCount++;
  dbgLastSetBlock = data->effectBlockIndex;
  dbgLastSetType = data->effectType;
...
{
    dbgConstantCount++;
    dbgLastMagnitude = data->magnitude;
//  ReportPrint(*effect);
  effect->magnitude = data->magnitude;  
}
...
void FfbReportHandler::FfbOnUsbData(uint8_t* data, uint16_t len)
{
  ...
  switch (data[0])    // reportID
  {
    ...
    if (len >= sizeof(USB_FFBReport_SetConstantForce_Output_Data_t)) {
      // HID is little-endian. STM32 is also little-endian, but decode it
      // explicitly to prove the raw packet contents.
      dbgRawConstantMagnitude =
