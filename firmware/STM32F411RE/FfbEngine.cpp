...
float deadBand = (float)effect.deadBand / 10000.0f;
float cpOffset = (float)effect.cpOffset / 10000.0f;
  ...
  if (metric < (cpOffset - deadBand)) {
    ...
    tempForce = ((float)1.00 * (cpOffset - deadBand) - metric) * negativeCoefficient;
...
//    tempForce =   (tempForce < (-effect.negativeCoefficient) ? (-effect.negativeCoefficient) : tempForce);
    ...
    tempForce = (metric - (float)1.00 * (cpOffset + deadBand)) * positiveCoefficient;
  ...
  tempForce = constrain(tempForce, -10000.0f, 10000.0f);

  tempForce = tempForce * 255.0f / 10000.0f;

  return (int32_t)tempForce;
}

volatile uint32_t dbgDamperCaseHits = 0;
volatile uint32_t dbgConstantCaseHits = 0;
...
int32_t FfbEngine::ForceCalculator(Encoder encoder)
{
  ...
  for (uint8_t id = 0; id <= MAX_EFFECTS; id++) {
      volatile TEffectState &effect = ffbReportHandler->gEffectStates[id]; // TODO replace this line with the actual get effect states
  ...
  if ((effect.state & MEFFECTSTATE_PLAYING) && ((effect.elapsedTime <= effect.duration) || (effect.duration == USB_DURATION_INFINITE)) && !ffbReportHandler->devicePaused)
      {
        ...
        ffbReportHandler->gEffectStates[id].elapsedTime = (uint64_t)millis()  - ffbReportHandler->gEffectStates[id].startTime; //TODO replace with my code
      }
    }
  force = (int32_t)((float)force * totalGainConfig / 10000.0f);
